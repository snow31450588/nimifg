...
def tile_sqlite(sqn_src, sqn_dst, level):
    ...
    for i in range(data_source.GetLayerCount()):
        ...
        fid = 0
        for col, row, tile_extent in tiles:
            geo_tile = gdal.ogr.CreateGeometryFromWkt(extent_to_wkt(tile_extent))
            sql = "select * from %s where ST_Intersects(GEOMETRY, GeomFromText('%s'))"%(table_name, extent_to_wkt(tile_extent))
            layer_read = data_source.ExecuteSQL(sql)
            #lon_min, lon_max, lat_max, lat_min = tile_extent
            #layer_read.SetSpatialFilterRect(lon_min, lat_min, lon_max, lat_max)
            layer_read.ResetReading()
            layer_write.StartTransaction()
            for feat in layer_read:
                geoms = geo_tile.Intersection(feat.GetGeometryRef())
                if not geoms:
                    continue

                if geoms.GetGeometryType() == geom_type:
                    geoms = [geoms]

                for geom in geoms:
                    if geom.GetGeometryType() != geom_type:
                        continue
                    #feature = feat.Clone()
